    ...
    private createSnake() {
        for (let i = 0; i < 4; i++) {
            ...
            dragBehavior.onDragStartObservable.add(() => {
                ...
                console.log(`Dragging started for box ${box.metadata.id}`);
                boxAggregate.body.disablePreStep = false; // Включаем предварительный шаг
                boxAggregate.body.setMassProperties({ mass: 10 }); // Оставляем кубик динамическим
            ...
            dragBehavior.onDragObservable.add((event) => {
                console.log(`Dragging box ${box.metadata.id}`, event.delta);
                // Обновляем целевую позицию физического тела
                const targetPosition = box.position.add(event.delta);
                const rotation = box.rotationQuaternion || Quaternion.Identity();
                boxAggregate.body.setTargetTransform(targetPosition, rotation);
            });

            dragBehavior.onDragEndObservable.add(() => {
                console.log(`Dragging ended for box ${box.metadata.id}`);
                // Сбрасываем скорость и угловую скорость
                boxAggregate.body.setLinearVelocity(Vector3.Zero());
                boxAggregate.body.setAngularVelocity(Vector3.Zero());
